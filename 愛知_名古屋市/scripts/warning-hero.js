...
(function (global) {
  ...
  var EVAC_ITEMS = [
    { group: 'evac', label: '避難情報', lv: 3, cls: 'warning', levelLabel: '',
      mid: '高齢者等避難', bot: '高齢者や避難に時間のかかる人は避難', badge: '発表中', kind: 'evac3' },
    { group: 'evac', label: '避難情報', lv: 4, cls: 'danger', levelLabel: '',
      mid: '避難指示', bot: '危険な場所から全員避難', badge: '発表中', kind: 'evac4' },
    { group: 'evac', label: '避難情報', lv: 5, cls: 'special', levelLabel: '',
      mid: '緊急安全確保', bot: '命を守るための最善の行動をとる', badge: '発表中', kind: 'evac5' }
  ];
  var CODE_MAP = {
    ...
    for (var i = 0; i < KINDS.length; i++) {
      ...
      for (var j = 0; j < LEVELS.length; j++) {
        ...
        out.push({
          ...
          mid: colorMidLabel_(lv.lv, kind.label, lv.key),
          bot: lv.bot,
          badge: '発表中'
        });
      }
  ...
  function liveMatchCodes(cityCode, areaCode) {
    ...
    var extra = {
      ...
      '1242734': [
        '120020',
        '1221000', '1221300', '1223900', '1224000',
        '1240361', '1240964', '1241046',
        '1242161', '1242253', '1242311', '1242471', '1242651'
      ],
  ...
  function makeTopSeg(scene) {
    ...
    if (scene && scene.levelLabel) addTopPart_(unit, 'top-label', scene.levelLabel);
  ...
  function midFontFamily_() {
    return '"Noto Sans JP","Yu Gothic UI","Yu Gothic",Meiryo,sans-serif';
  }

  function fitMidToUnit_(el) {
    if (!el) return;
    var text = midChipLabel_(el);
    var n = Math.max(1, String(text).length);
    var maxW = UNIT_W - 8;
    var size = Math.min(48, Math.floor(maxW / n));
    var fam = midFontFamily_();
    var guard = 80;
    while (guard-- > 0 && size > 18) {
      var w = canvasTextWidth(text, '900 ' + size + 'px ' + fam);
      if (w <= maxW - 4) break;
      size -= 0.5;
    }
  ...
  function shrinkUntilFits_(el, maxW, maxH, minPx) {
    ...
    var measured = elWide_(el);
    if (maxW > 0 && measured < 2) return;
    var guard = 32;
    while (guard-- > 0) {
  ...
  function fillBotLines(el, text) {
    ...
    var parts = splitBot2_(stripBotParen_(text)).split('\n').filter(function (p) { return !!p; });
    for (var i = 0; i < parts.length; i++) {
  ...
  function fitToWidth(el, maxW, maxPx) {
    ...
    var budget = Math.max(40, maxW);
    var cap = maxPx;
    var px = pxByChars_(budget, n, cap, 0);
    if (isMid && n <= 2) el.style.letterSpacing = '0.04em';
    ...
    shrinkUntilFits_(el, budget, isMid ? 48 : 0, isMid ? 18 : 10);
    el.style.maxWidth = maxW + 'px';
  }
  ...
  function fitBot(el, maxH, maxW) {
    ...
    maxW = maxW || (UNIT_W - 16);
    maxH = maxH || 38;
    var lines = el.querySelectorAll('.bot-line');
    var lineCount = Math.max(1, lines.length);
    var lineH = Math.floor(maxH / lineCount);
    var n = longestLineLen_(el.textContent || '');
    var cap = lineCount === 1 ? Math.min(26, maxH) : Math.min(20, lineH);
    var size = pxByChars_(maxW, n, cap, 0);
    ...
    for (var i = 0; i < lines.length; i++) {
      ...
      lines[i].style.overflow = 'visible';
      lines[i].style.textAlign = 'center';
    ...
    while (guard-- > 0) {
      ...
      for (var j = 0; j < lines.length; j++) {
        if (elWide_(lines[j]) > maxW) tooW = true;
      }
      var tooH = (el.scrollHeight || 0) > maxH;
      if (!tooW && !tooH) break;
      if (size <= 12) break;
      size -= 0.5;
      el.style.fontSize = size + 'px';
  ...
  function measureMidWidth(mid) {
    ...
    var font = '900 48px "Noto Sans JP","Yu Gothic UI","Yu Gothic",Meiryo,sans-serif';
    var letter = 0;
    var stroke = 6;
  ...
  function afterLayout(cb) {
    ...
    document.fonts.load('900 48px "Noto Sans JP"').then(once, once);
    setTimeout(once, 80);
  }
  ...
  function createPlayer(opts) {
    ...
    function renderItem(index) {
      ...
      [topTrack, midTrack, botTrack].forEach(function (track) {
        if (track) track.style.transform = 'translate3d(0,0,0)';
      });
      var firstWs = [prepareTrack(topTrack), prepareTrack(midTrack), prepareTrack(botTrack)];
      [topTrack, midTrack, botTrack].forEach(function (track, i) {
        if (!track || !(firstWs[i] > 0)) return;
        track.style.transform = 'translate3d(0,0,0)';
      });
